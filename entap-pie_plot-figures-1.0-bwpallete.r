...
bwpallete <- c("grey90","grey50","black","grey30","white","grey70","grey50","grey20","grey40","grey60")
...
plot<-ggplot(objs3[[i]]) +
  ...
  geom_label_repel(aes(x = sin(middle+1), y = cos(middle+1), label = Label, hjust = NULL, vjust = vjust/10), nudge_x= 0.5 * cos(objs3[[i]]$middle), nudge_y = 0.1 * cos(objs3[[i]]$middle), cex = 7, fill = rev(bwpallete), fontface = "bold") + 
  coord_fixed() +
  ...
  scale_y_continuous(limits = c(-1.01, 1.01),      # Adjust so labels are not cut off
                     name = "", breaks = NULL, labels = NULL) +
  labs(fill="Species") + 
  scale_fill_manual(values = bwpallete) + #custom fill for pie areas
